...
module.exports = class Student {
    ...
    async enroll({ schoolId, classroomId, students, __schoolAdmin }) {
        try {
            ...
            if (availableCapacity < students.length) {
                return {
                    error: `Classroom has only ${availableCapacity} available slots, but ${students.length} students provided`
                };
            }
            ...
            for (let i = 0; i < students.length; i++) {
                ...
                const age = Math.floor(
                    (Date.now() - new Date(studentData.dateOfBirth)) /
                        (365.25 * 24 * 60 * 60 * 1000)
                );
                if (age < classroom.minAge || age > classroom.maxAge) {
                    errors.push({
                        index: i,
                        error: `Student age must be between ${classroom.minAge} and ${classroom.maxAge} years for this classroom`
                    });
                    continue;
                }
            ...
            if (enrolledStudents.length > 0) {
                await this.ClassroomModel.findByIdAndUpdate(classroomId, {
                    $inc: { currentEnrollment: enrolledStudents.length }
                });
            }
            ...
            this.logger.error(
                { error: err.message, schoolId, classroomId },
    ...
    async update({
        ...
        try {
            ...
            if (
                ...
                __schoolAdmin.schoolId !== student.schoolId.toString()
            ) {
                return { error: 'Student not found', code: 404 };
            }
            ...
            this.logger.info(
                { studentId, updates: Object.keys(updateData) },
    ...
    async transfer({ studentId, toSchoolId, toClassroomId, reason, __schoolAdmin }) {
        try {
            const { error } = this.validators.transfer.validate({
                studentId,
                toSchoolId,
                toClassroomId,
                reason
            ...
            if (
                ...
                !mongoose.Types.ObjectId.isValid(toSchoolId) ||
                !mongoose.Types.ObjectId.isValid(toClassroomId)
            ) {
            ...
            if (
                student.schoolId.toString() === toSchoolId &&
                student.classroomId.toString() === toClassroomId
            ) {
            ...
            const toClassroom = await this.ClassroomModel.findOne({
                _id: toClassroomId,
                schoolId: toSchoolId,
                deletedAt: null
            });
            ...
            const studentAge = Math.floor(
                (Date.now() - new Date(student.dateOfBirth)) / (365.25 * 24 * 60 * 60 * 1000)
            );
            if (studentAge < toClassroom.minAge || studentAge > toClassroom.maxAge) {
                return {
                    error: `Student age must be between ${toClassroom.minAge} and ${toClassroom.maxAge} years for target classroom`
                };
            }
            ...
            await student.save();
            await this.ClassroomModel.findByIdAndUpdate(fromClassroomId, {
                $inc: { currentEnrollment: -1 }
            });
            await this.ClassroomModel.findByIdAndUpdate(toClassroomId, {
                $inc: { currentEnrollment: 1 }
            });
    ...
    async list({ schoolId, classroomId, page = 1, limit = 10, isActive, __schoolAdmin }) {
        try {
            const { error } = this.validators.list.validate({
                schoolId,
                classroomId,
                page,
                limit,
    ...
    async getById({ studentId, __schoolAdmin }) {
        try {
            ...
            if (
                __schoolAdmin.role === 'school_admin' &&
                __schoolAdmin.schoolId !== student.schoolId._id.toString()
            ) {
                return { error: 'Student not found', code: 404 };
            }
